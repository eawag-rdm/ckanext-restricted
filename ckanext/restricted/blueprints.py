...
def _send_request_mail(data, resource_id):
    ...
    recip_name = Header(
        '"{}"'.format(data.get('maintainer_name')),
        'utf-8').encode('utf-8')
    ...
    body = render_jinja2(access_mail_template, extra_vars)
    body_user = ('Please find below a copy of the access request'
    ...
    recip_name = Header(
        '"{}"'.format(data.get('user_name')), 'utf-8').encode('utf-8')
    try:
...
def _send_request(context):
    ...
    if data_dict.get('message', '') == '':
        errors['message'] = ['Missing Value']
        error_summary['message'] =  'Missing Value'
    ...
    error_summary = _send_request_mail(data_dict, resource_id)
...
def restricted_request_access_form(package_id, resource_id, data={},
    ...
    if not data:
        ...
        contact_details = _get_contact_details(data['pkg_dict'])
